...
if newer(cSources, nativeSearchLib):
  ...
  run('g++ -g -fPIC -O3 -ffast-math -shared -o %s -I%s/include -I%s/include/linux %s' % (nativeSearchLib, JAVA_HOME, JAVA_HOME, ' '.join(cSources)))
...
if newer(mmapSource, mmapLib):
  ...
  run('g++ -g -fPIC -O3 -shared -o %s -I%s/include -I%s/include/linux %s' % (mmapLib, JAVA_HOME, JAVA_HOME, mmapSource))
